...
export class ValueRealizationController {
  ...
  public exportCsv = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const headers = ['recommendationId', 'title', 'provider', 'serviceName', 'resourceId', 'currency', 'estimatedMonthlySavings', 'reportedMonthlySavings', 'observedSavings', 'projectedMonthlySavings', 'verifiedMonthlySavings', 'costIncreaseMonthlyAmount', 'measurementStatus', 'nextAction', 'executedAt', 'verifiedAt'];
      const rows = items.map((item) => [item.recommendationId, item.title, item.provider, item.serviceName ?? '', item.resourceId ?? '', item.currency, item.estimatedMonthlySavings, item.reportedMonthlySavings, item.observedSavings ?? '', item.projectedMonthlySavings ?? '', item.verifiedMonthlySavings, item.costIncreaseMonthlyAmount, item.measurementStatus ?? '', item.nextAction, item.executedAt?.toISOString() ?? '', item.verifiedAt?.toISOString() ?? '']);
