...
export class BudgetController {
  ...
  private async run(req: Request, res: Response, operation: () => Promise<unknown>, status = 200): Promise<void> { try { res.status(status).json({ success: true, ...(await operation() as object) }); } catch (error) { const known = error instanceof FinOpsBaseError; const code = known ? error.code : 'INTERNAL_ERROR'; const http = code === 'AUTHENTICATION_REQUIRED' ? 401 : code === 'AUTHORIZATION_FAILED' ? 403 : code === 'NOT_FOUND' ? 404 : code === 'VALIDATION_ERROR' ? 400 : 500; const diagnosticId = typeof res.locals.requestId === 'string' ? res.locals.requestId : undefined; if (!known) console.error(JSON.stringify({ level: 'error', event: 'budget_operation_failed', diagnosticId, path: req.path, error: safeErrorMessage(error) })); res.status(http).json({ success: false, code, error: known ? error.message : 'No fue posible operar con presupuestos.', ...(diagnosticId === undefined ? {} : { diagnosticId }) }); } }
}
function string(value: unknown): string | undefined { return typeof value === 'string' && value.trim() !== '' ? value.trim() : undefined; }
function requiredParam(value: unknown): string { const parsed = string(value); if (parsed === undefined) throw new FinOpsBaseError('Falta el identificador del presupuesto', 'VALIDATION_ERROR'); return parsed; }
