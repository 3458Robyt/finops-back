...
export class BudgetController {
  ...
  private async run(req: Request, res: Response, operation: () => Promise<unknown>, status = 200): Promise<void> { try { res.status(status).json({ success: true, ...(await operation() as object) }); } catch (error) { respondWithFinOpsError(res, error, 'No fue posible operar con presupuestos.', 'budget_operation_failed', req.path); } }
}
