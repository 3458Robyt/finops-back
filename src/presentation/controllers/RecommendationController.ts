...
export class RecommendationController {
  ...
  public getSavingsMeasurementReadiness = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      const recommendationId = requireRecommendationId(res, req.params['id']);
      if (recommendationId === undefined) return;
      const readiness = await this.recommendationRepository.getSavingsMeasurementReadiness(auth.tenantId, recommendationId);
      res.status(200).json({ success: true, readiness });
    } catch (error: unknown) {
      respondWithRecommendationError(res, error, 'An unexpected error occurred loading savings measurement readiness');
    }
  };

  public createSavingsMeasurement = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      if (!requireRecommendationExecutionRole(res, auth)) return;
      ...
      if (recommendationId === undefined || manualExecutionId === undefined) {
        res.status(400).json({ success: false, error: 'El id de la recomendación y manualExecutionId son obligatorios', code: 'VALIDATION_ERROR' });
        return;
      }
  ...
  public listSavingsMeasurements = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      const recommendationId = requireRecommendationId(res, req.params['id']);
      if (recommendationId === undefined) return;
      const measurements = await this.recommendationRepository.findSavingsMeasurementsByRecommendation(auth.tenantId, recommendationId);
      res.status(200).json({ success: true, measurements, meta: { count: measurements.length } });
    } catch (error: unknown) {
      respondWithRecommendationError(res, error, 'No fue posible cargar las mediciones de ahorro');
    }
  };

  public getSavingsMeasurement = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      const recommendationId = requireRecommendationId(res, req.params['id']);
      ...
      const measurement = await this.recommendationRepository.findSavingsMeasurementById(auth.tenantId, recommendationId, measurementId);
      if (measurement === null) {
        res.status(404).json({ success: false, error: 'No se encontró la medición de ahorro', code: 'NOT_FOUND' });
        return;
      }
      res.status(200).json({ success: true, measurement });
    } catch (error: unknown) {
      respondWithRecommendationError(res, error, 'No fue posible cargar la medición de ahorro');
    }
  };

  public verifySavingsMeasurement = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      ...
      if (recommendationId === undefined || measurementId === undefined) {
        res.status(400).json({ success: false, error: 'El id de la recomendación y de la medición son obligatorios', code: 'VALIDATION_ERROR' });
        return;
      }
      const note = parseString(readBodyValue(req.body, 'note'));
      const measurement = await this.recommendationRepository.verifySavingsMeasurement({
        tenantId: auth.tenantId,
        recommendationId,
        measurementId,
        userId: auth.userId,
  ...
  public rejectSavingsMeasurement = async (req: Request, res: Response): Promise<void> => {
    try {
      const auth = requireAuth(req, res);
      if (auth === undefined) return;
      if (!requireRecommendationDecisionRole(res, auth)) return;
      ...
      if (recommendationId === undefined || measurementId === undefined || reason === undefined) {
        res.status(400).json({ success: false, error: 'El id de la recomendación, de la medición y el motivo son obligatorios', code: 'VALIDATION_ERROR' });
        return;
      }
      const measurement = await this.recommendationRepository.rejectSavingsMeasurement({
        tenantId: auth.tenantId,
        recommendationId,
        measurementId,
        userId: auth.userId,
      ...
      respondWithRecommendationError(res, error, 'No fue posible rechazar la medición de ahorro');
    }
  };
