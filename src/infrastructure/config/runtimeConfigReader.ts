...
export function loadRuntimeConfig(env: NodeJS.ProcessEnv = process.env): RuntimeConfig {
  ...
  return {
    ...
    workers: {
      ...
      metricProjection: {
        enabled: readBoolean(env['METRIC_PROJECTION_WORKER_ENABLED'], false),
        id: readOptionalString(env['METRIC_PROJECTION_WORKER_ID']),
    ...
    schedulers: {
      ...
      ingestion: {
        ...
        maxAttempts: readPositiveInteger(env['INGESTION_SCHEDULER_MAX_ATTEMPTS'], 3),
        metricCatchupDays: readPositiveInteger(env['INGESTION_SCHEDULER_METRIC_CATCHUP_DAYS'], 90),
        metricCatchupWindowMinutes: readPositiveInteger(env['INGESTION_SCHEDULER_METRIC_CATCHUP_WINDOW_MINUTES'], 24 * 60),
        maxMetricBackfillJobsPerConnection: readPositiveInteger(env['INGESTION_SCHEDULER_MAX_METRIC_BACKFILL_JOBS_PER_CONNECTION'], 48),
