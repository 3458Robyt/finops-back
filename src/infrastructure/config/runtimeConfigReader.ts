...
export function loadRuntimeConfig(env: NodeJS.ProcessEnv = process.env): RuntimeConfig {
  ...
  return {
    ...
    ai: {
      apiKey: readOptionalString(env['AI_API_KEY'] ?? env['NVIDIA_API_KEY'] ?? env['NIM_API_KEY']),
      baseUrl: readString(env['AI_BASE_URL'] ?? env['NVIDIA_BASE_URL'], 'https://api.openai.com/v1'),
      model: readString(env['AI_MODEL'] ?? env['NVIDIA_MODEL'], 'gpt-5.4-mini'),
      auditorModel: readString(env['AI_AUDITOR_MODEL'] ?? env['NVIDIA_AUDITOR_MODEL'], readString(env['AI_MODEL'] ?? env['NVIDIA_MODEL'], 'gpt-5.4-mini')),
