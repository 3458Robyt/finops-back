...
class ConcurrentMfaChallengeUpdate extends Error {}
    ...
    try {
      return await this.prisma.$transaction(async (transaction) => {
        ...
        if (
          ...
          || challenge.purpose !== (input.enableMfa ? 'ENROLLMENT' : 'LOGIN')
        ...
        if (challengeUpdate.count !== 1) throw new ConcurrentMfaChallengeUpdate();
        return true;
      });
    } catch (error: unknown) {
      if (error instanceof ConcurrentMfaChallengeUpdate) return false;
      throw error;
    }
  }
}
