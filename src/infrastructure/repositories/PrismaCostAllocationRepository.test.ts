...
describe('cost allocation summarization', () => {
  it('uses the first matching priority, never double-counts, and preserves currency boundaries', () => {
    ...
    expect(result.find((item) => item.currency === 'USD')).toMatchObject({ totalCost: 10, allocatedCost: 10, unallocatedCost: 0, dimensions: [{ allocationKey: 'CC-A', cost: 10, percentage: 100 }] });
