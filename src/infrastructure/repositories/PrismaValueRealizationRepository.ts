  ...
  public async listDestinationSummary(input: { readonly tenantId: string; readonly period: Date; readonly currency?: string }): Promise<readonly ValueRealizationDestinationSummary[]> {
    ...
    const rows = await this.prisma.$queryRaw<Array<ValueRealizationRow>>(Prisma.sql`
      WITH closed_periods AS (
        SELECT c.*,
               ROW_NUMBER() OVER (PARTITION BY c.tenant_id, c.period_start, c.currency ORDER BY c.version DESC) AS closure_rn
        ...
        SELECT m.*,
               ROW_NUMBER() OVER (PARTITION BY m.manual_execution_id ORDER BY CASE WHEN m.status = 'VERIFIED' THEN 0 WHEN m.status = 'REJECTED' THEN 2 ELSE 1 END, m.created_at DESC, m.id DESC) AS measurement_rn
        ...
        INNER JOIN cost_allocation_closure_lines l
          ON l.tenant_id = r.tenant_id
          AND l.currency = r.currency
          AND l.cloud_resource_id = r.cloud_resource_id
          AND l.metric_identity_hash = r.source_metric_identity_hash
          AND l.charge_period_start = r.source_charge_period_start
          AND l.source_amount > 0
        INNER JOIN closed_periods c
          ON c.id = l.closure_id
         AND c.closure_rn = 1
         AND r.source_charge_period_start >= c.period_start
         AND r.source_charge_period_start < c.period_start + INTERVAL '1 month'
      ...
      SELECT ${period.toISOString().slice(0, 7)} AS period,
             allocation_key,
             currency,
      ...
      FROM attributed
      GROUP BY allocation_key, currency
      ORDER BY currency ASC, allocation_key ASC
    `);
    return rows.map((row) => ({ period: stringValue(row['period']) ?? period.toISOString().slice(0, 7), allocationKey: stringValue(row['allocation_key']) ?? 'UNALLOCATED', currency: stringValue(row['currency']) ?? 'USD', potentialSavings: numberValue(row['potential_savings']), approvedSavings: numberValue(row['approved_savings']), verifiedSavings: numberValue(row['verified_savings']), observedSavings: numberValue(row['observed_savings']), attributedRecommendations: intValue(row['attributed_recommendations']) }));
  }
