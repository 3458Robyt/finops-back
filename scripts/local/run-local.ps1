...
$env:APP_PROCESS_ROLE = if ($Mode -eq 'dev') { 'api' } elseif ($Mode -eq 'worker') { 'ingestion-worker' } else { 'ingestion-scheduler' }
...
$env:METRIC_PROJECTION_WORKER_ENABLED = if ($Mode -eq 'worker') { 'true' } else { 'false' }
