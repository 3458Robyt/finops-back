...
const schema = `finops_e2e_cost_allocation_${Date.now().toString(36)}`;
...
try {
  ...
  const result = await runCommand(process.execPath, [vitestCli, 'run', '--disableConsoleIntercept', 'src/testing/costAllocation.integration.test.ts'], {
