...
> Registro autoritativo del corte **2026-09-04**. Este archivo contiene únicamente
...
| AI-002 | Media | IA/gobernanza | ABIERTO | Mantener una calificación periódica del proveedor real con escenarios dorados, auditoría, evidencia y abstención ante datos débiles; reducir la latencia live antes de escalar. | El canary de análisis anterior 2026-09-04 terminó 3/3 aprobado; el canary ampliado de chat/planes confirmó Markdown seguro y abstención técnica, pero obtuvo recomendaciones 2/3 (una respuesta HTTP 500) y rechazo del auditor en 3/3 planes (HTTP 409), sin persistir planes no aprobados. Investigar calidad/latencia del plan y disponibilidad del proveedor; conservar latencia, tokens y motivos sanitizados. |
...
| PERF-004 | Alta | Ingesta técnica | ABIERTO | Establecer SLO de persistencia y throughput para backfills grandes con límites OCI, sin descartar raw ni estadísticas. | El raw-first, streaming, batching, cuatro workers y projection worker asíncrono están implementados. La importación fría OCI quedó en mediana ~2,3 s; falta medir cierre completo de backfill en el destino elegido y ajustar concurrencia con métricas reales de OCI/BD. |
