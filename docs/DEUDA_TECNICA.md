...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` bajó de 1.140 a 393 líneas; `CloudConnectionService` pasó de 979 a una fachada de 148 líneas sobre módulos de máximo 400, preservando 23 pruebas de caracterización. Aún requieren extracción incremental los repositorios grandes, `TechnicalMetricsService`, `CloudConnectionController`, DTO catalog y `ResourceDetail`. |
