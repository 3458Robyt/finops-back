...
| AUTH-003 | Alta | Autorización | CERRADO | Evitar listas de roles divergentes y probar de forma exhaustiva quién puede ejecutar cada operación sensible. | `AuthorizationPolicy` define siete roles y dieciséis capacidades; rutas y servicios de cloud, ingesta, agente, recomendaciones, ahorro, presupuestos, asignación, valor, mensajería, MFA y administración MSP reutilizan la política. Matriz en `docs/MATRIZ_AUTORIZACION.md`; suite dirigida 45/45 y `test:all` 301/301 aprobadas. |
...
| MOD-001 | Media | Mantenibilidad | DIFERIDO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Auth API, transporte frontend por dominio, políticas cloud, rate limit, observabilidad, aprendizaje y paneles de `AgentSettings` ya están aislados; `AgentLearningService` quedó en 130 líneas efectivas y la vista de configuración en 396. `OciSdkIngestionProvider`, `CloudConnectionService`, repositorios grandes, DTO catalog y `ResourceDetail` requieren extracción incremental con pruebas de regresión; no se hará una reescritura riesgosa en esta beta. |
