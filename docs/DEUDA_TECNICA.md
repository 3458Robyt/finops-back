...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` bajó de 1.140 a 393 líneas al extraer contratos, validación, Monitoring, compartimentos, inventario, FOCUS y retries en módulos de máximo 219 líneas, con 22/22 pruebas OCI dirigidas. Aún requieren extracción incremental `CloudConnectionService`, repositorios grandes, DTO catalog y `ResourceDetail`. |
