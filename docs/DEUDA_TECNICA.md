...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` quedó en 395 líneas, `CloudConnectionService` en 148, `TechnicalMetricsService` en 202, `PrismaResourceLinkageReadinessRepository` en 360 y `Ingesta.tsx` en 363; sus queries/paneles se separaron por responsabilidad. Aún requieren extracción incremental otros repositorios grandes, `CloudConnectionController`, DTO catalog y `ResourceDetail`. |
