...
| DB-001 | Alta | Supabase | CERRADO | Hardening de funciones y cobertura de índices FK. | 43 migraciones al día; Advisors seguridad sin lints; 27 índices FK presentes; tablas de asignación sin acceso API directo. |
...
| PERF-001 | Media | Rendimiento | DIFERIDO | Llevar preview y cierre de asignación a los objetivos orientativos de 500 ms y 2 s con 10.000 costos persistidos, cuando exista un entorno representativo de despliegue. | Supabase actual: preview 1.437,96 ms y cierre 4.800,78 ms; 10.000 líneas persistidas sin pérdida ni descuadre. Insert masivo JSONB y timeout ampliado ya aplicados; falta medir con `EXPLAIN (ANALYZE, BUFFERS)` y ajustar índices/infraestructura sin degradar la trazabilidad. |
