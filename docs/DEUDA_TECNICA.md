...
| QA-002 | Media | Validación UI | CERRADO | Smoke autenticado reproducible sin depender de una contraseña real compartida. | `test:e2e:full` aplica 52 migraciones en un schema `finops_e2e_*`, crea credenciales controladas, aprobó 5/5 flujos Playwright y elimina fixtures/schema en `finally`; el cleanup también tolera schemas incompletos. |
...
| AUTH-003 | Alta | Autorización | CERRADO | Evitar listas de roles divergentes y probar de forma exhaustiva quién puede ejecutar cada operación sensible. | `AuthorizationPolicy` define siete roles y dieciséis capacidades; rutas y servicios de cloud, ingesta, agente, recomendaciones, ahorro, presupuestos, asignación, valor, mensajería, MFA y administración MSP reutilizan la política. Matriz en `docs/MATRIZ_AUTORIZACION.md`; suite dirigida 45/45 y `test:all` 312/312 aprobadas. |
...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` quedó en 395 líneas, `CloudConnectionService` en 148, `TechnicalMetricsService` en 202, `PrismaResourceLinkageReadinessRepository` en 360, `Ingesta.tsx` en 363 y `ResourceDetail.tsx` en 348; sus queries, controladores y paneles se separaron por responsabilidad. Aún requieren extracción incremental otros repositorios grandes, `CloudConnectionController`, DTO catalog y `MetricasTecnicas.tsx`. |
