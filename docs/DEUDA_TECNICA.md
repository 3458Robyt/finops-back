...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` bajó a 393 líneas; `CloudConnectionService` a una fachada de 148 y `TechnicalMetricsService` a 202, con módulos internos de máximo 400 y sus pruebas de caracterización verdes. Aún requieren extracción incremental los repositorios grandes, `CloudConnectionController`, DTO catalog y `ResourceDetail`. |
