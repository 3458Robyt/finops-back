...
| PERF-001 | Media | Rendimiento | DIFERIDO | Llevar preview y cierre de asignación a los objetivos orientativos de 500 ms y 2 s con 10.000 costos persistidos, cuando exista un entorno representativo de despliegue. | Supabase actual: preview 1.470,90 ms y cierre 4.560,54 ms; 10.000 líneas persistidas sin pérdida ni descuadre. `EXPLAIN (ANALYZE, BUFFERS)` usa `cost_metrics_tenant_period_idx` y tarda 9,704 ms en el plan; insert masivo JSONB y timeout ampliado ya aplicados. Falta ajustar transferencia/infraestructura sin degradar la trazabilidad. |
