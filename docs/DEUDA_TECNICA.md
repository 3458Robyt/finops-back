...
| AUTH-003 | Alta | Autorización | CERRADO | Evitar listas de roles divergentes y probar de forma exhaustiva quién puede ejecutar cada operación sensible. | `AuthorizationPolicy` define siete roles y dieciséis capacidades; rutas y servicios de cloud, ingesta, agente, recomendaciones, ahorro, presupuestos, asignación, valor, mensajería, MFA y administración MSP reutilizan la política. Matriz en `docs/MATRIZ_AUTORIZACION.md`; suite dirigida 45/45 y `test:all` 307/307 aprobadas. |
...
| MOD-001 | Media | Mantenibilidad | ABIERTO | Continuar dividiendo hotspots mayores de 200 líneas sin introducir duplicación ni cambios de contrato. | Autorización, auth API, transporte frontend, políticas cloud, rate limit, observabilidad y aprendizaje están aislados. `OciSdkIngestionProvider` bajó de 1.140 a 779 líneas al extraer contratos, validación y Monitoring en módulos de 111/219/133 líneas, con 13/13 pruebas OCI dirigidas. Aún requieren extracción incremental inventario/FOCUS, `CloudConnectionService`, repositorios grandes, DTO catalog y `ResourceDetail`. |
